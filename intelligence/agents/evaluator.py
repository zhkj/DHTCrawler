"""
LLM-as-Judge 评估模块
对 Agent 回答质量进行自动评分，用于持续优化。
评估维度：相关性、完整性、准确性、信息源引用。
"""
import logging
from datetime import datetime
from openai import OpenAI
from config import LLM_API_KEY, LLM_BASE_URL, LLM_MODEL
from db.mongo_client import save_evaluation

logger = logging.getLogger("intelligence.evaluator")
...
def evaluate(query: str, response: str, tool_chain: str = "",
    ...
    """
    对一次 Agent 回答进行 LLM-as-Judge 评估。
    返回评估结果 dict，或 None（评估失败时）。
    """
    ...
    try:
        ...
        logger.info(
            f"[Evaluator] trace={trace_id} avg={avg_score} "
            f"scores={scores}"
        )
    except Exception as e:
