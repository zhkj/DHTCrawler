"""
Agent 可观测性 — Trace 记录
记录每次 Agent 调用的完整链路：工具调用、耗时、token 用量。
存入 MongoDB traces 集合，供 UI 展示和事后分析。
"""
import time
import logging
from datetime import datetime
from db.mongo_client import save_trace

logger = logging.getLogger("intelligence.tracer")
...
class Trace:
    ...
    def add_tool_call(self, tool_name: str, tool_input: dict, result_preview: str,
                      latency_ms: int):
        """记录一次工具调用。"""
    ...
    def finish(self, response: str = "", status: str = "success", error: str = None):
        """完成 trace，计算总耗时并持久化。"""
