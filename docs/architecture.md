...
| 组件 | 选择 | 说明 |
|---|---|---|
| LLM | OpenAI 兼容 API | 通过 `openai` SDK 调用 |
| Agent 框架 | 手写 Orchestrator | `src/intelligence/core/orchestrator.py`，ReAct 模式 |
| Vector DB | ChromaDB（本地） | `src/intelligence/rag/vectorstore.py` |
| Embedding | `sentence-transformers` | 免费、离线、多语言支持 |
| 短期记忆 | Python 内存 | `src/intelligence/core/memory/conversation.py` |
| 长期记忆 | MongoDB | `src/intelligence/core/memory/long_term.py` |
| Web UI | Streamlit | `src/intelligence/web/`，含对话、HITL、可观测性页面 |
| 通知 | 钉钉 / 飞书 / Telegram | `src/intelligence/notify/` |
| 可观测性 | 自研 Tracer + Evaluator | `src/intelligence/observability/` |
...
| 方向 | 状态 | 代码位置 |
|---|---|---|
| RAG 语义搜索 | ✅ 已实现 | `src/intelligence/rag/` |
| Multi-Agent + Tool Use | ✅ 已实现 | `src/intelligence/core/orchestrator.py`, `tools.py` |
| Memory 两级架构 | ✅ 已实现 | `src/intelligence/core/memory/` |
| 可观测性与评估 | ✅ 已实现 | `src/intelligence/observability/` |
| Human-in-the-Loop | ✅ 已实现 | `src/intelligence/monitor/hitl.py`, `web/pages/hitl.py` |
| Monitor Agent | ✅ 已实现 | `src/intelligence/monitor/agent.py` |
| 多渠道通知 | ✅ 已实现 | `src/intelligence/notify/` |
| 外部数据源补全 | ✅ 已实现 | `src/intelligence/enrichment/` |
| 流式 / 实时处理 | 🔲 待实现 | — |
| 知识图谱 | 🔲 待实现 | — |
| 自适应资源调度 | 🔲 待实现 | — |
