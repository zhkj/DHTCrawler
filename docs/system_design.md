...
┌──────────────────────────────────────────────────────┐
│                    用户层                             │
│  Streamlit Web UI（对话、HITL 审查、可观测性仪表盘）  │
│  src/intelligence/web/                               │
└──────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────┐
│                    Agent 层                           │
│  LLM:       OpenAI 兼容 API                          │
│  框架:      手写 ReAct Orchestrator                   │
│  工具调用:  Function Calling                          │
│  监控:      Monitor Agent + 置信度评分 + HITL         │
│  通知:      钉钉 / 飞书 / Telegram                   │
└──────────────────────────────────────────────────────┘
...
│  可观测性:  自研 Tracer + LLM-as-Judge Evaluator     │
└──────────────────────────────────────────────────────┘
...
│                    存储层                             │
│  主数据库:   MongoDB（复用现有）                       │
│  短期记忆:   Python 内存（对话上下文）                 │
│  长期记忆:   MongoDB（用户偏好、历史查询）             │
│  向量存储:   ChromaDB                                 │
└──────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────┐
│                    采集层                              │
│  DHT Crawler: src/crawler/（Python 3，asyncio）      │
│  协议实现:   KRPC + BEP-9 + BEP-51 + BEP-15         │
└──────────────────────────────────────────────────────┘
```
...
| 模块 | 用到的 Agent 能力 | 状态 |
|---|---|---|
| Enrichment | Tool Use（多源 API）、并发执行、结果合并 | ✅ |
| RAG 模块 | RAG（向量检索） | ✅ |
| Memory 层 | Short-term Memory（滑动窗口/摘要）、Long-term Memory、用户偏好 | ✅ |
| Orchestrator | 意图理解、ReAct 规划、工具调用路由 | ✅ |
| Monitor Agent | 告警监听、置信度评分、Human-in-the-Loop | ✅ |
| 通知系统 | 多渠道分发（钉钉/飞书/Telegram） | ✅ |
| 可观测性 | 链路追踪（Tracer）、LLM-as-Judge 评估 | ✅ |
...
✅ 阶段 1：数据管道打通
  ├─ DHT 数据 → MongoDB
  ├─ Enrichment 工具：HN API + Reddit API + RSS
  └─ src/intelligence/enrichment/

✅ 阶段 2：RAG + 基础 Agent
  ├─ 数据向量化存入 ChromaDB（src/intelligence/rag/）
  ├─ Orchestrator Agent：自然语言 → 工具调用 → 生成回复
  └─ 上下文管理：多轮对话状态跟踪（src/intelligence/core/memory/）

✅ 阶段 3：Multi-Agent + Tool Use
  ├─ Orchestrator + 工具集（src/intelligence/core/）
  └─ Tool Use：完整工具集接入（src/intelligence/core/tools.py）

✅ 阶段 4：Memory + Monitor + HITL
  ├─ 两级记忆（内存短期 + MongoDB 长期）
  ├─ Monitor Agent + 置信度评分（src/intelligence/monitor/）
  ├─ Human-in-the-Loop 审查流程（src/intelligence/monitor/hitl.py）
  └─ 多渠道通知：钉钉 / 飞书 / Telegram（src/intelligence/notify/）

✅ 阶段 5：可观测性 + Web UI
  ├─ 链路追踪 + LLM-as-Judge 评估（src/intelligence/observability/）
  ├─ Streamlit Web UI（对话、HITL、可观测性页面）
  └─ 项目工程化：pyproject.toml、Makefile、tests/

🔲 待实现：流式/实时处理、知识图谱、自适应资源调度
```
